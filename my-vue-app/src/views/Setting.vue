          ...
          >
            <ion-card class="card-card" @click="showCardDetails(card)">
              ...
              <ion-item lines="none" class="card-actions">
                <ion-button fill="outline" color="primary" class="buy-btn" @click.stop="buyCard(card.id)">
                  Купить
                </ion-button>
...
const getCollections = async () => {
  try {
    const response = await apiFetch('http://localhost:3000/api/getCollections');
    cards.value = response || [];
    cards.value = await enrichCards(cards.value, assets);
  } catch (error) {
...
const getUnits = async () => {
  try {
    ...
    // Преобразуем каждую порцию юнитов и собираем все в один массив
...
function enrichCards(cards, assets) {
  const assetMap = new Map(assets.map(a => [a.key, a.url]));
  return cards.map(card => ({
    ...card,
    image_url: assetMap.has(card.image_url) ? assetMap.get(card.image_url) : card.image_url
  }));
}
...
// открыть модальное окно юнитов для карты
...
const buyCard = async (card) => {
  try {
    const response = await apiFetch('http://localhost:3000/api/buyCard', {
      method: 'POST',
      body: JSON.stringify({
    ...
    console.error('Ошибка покупки карты:', err);
...
const showToast = (text) => {
  toast.value = { open: true, message: text };
  setTimeout(()=>{toast.value=false},1300);
};
