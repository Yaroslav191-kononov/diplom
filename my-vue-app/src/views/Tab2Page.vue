  ...
  <ion-page>
    ...
    <ion-header class="ion-no-border">
      <ion-toolbar class="glass-toolbar">
        <ion-title class="auth-title">Вход в систему</ion-title>
      </ion-toolbar>
    </ion-header>

    <ion-content fullscreen class="auth-content ion-padding">
      <!-- Стеклянная карточка авторизации лежит напрямую в контенте -->
      <div class="glass-auth-card">
        <h2 class="welcome-text">Добро пожаловать</h2>
        <p class="welcome-subtext">Введите ваши данные для доступа к игре</p>
        ...
        <form @submit.prevent="onSubmit" class="auth-form">
          ...
          >
            {{ loading ? 'Загрузка...' : 'Войти на Арену' }}
          </ion-button>
        </form>
      </div>
    </ion-content>
...
const onSubmit = async () => {
  ...
  loading.value = true;
  try {
    const response = await apiFetch('http://localhost:3000/api/check', {
      method: 'POST',
      mode: 'cors',
      credentials: 'include',
    ...
    if (!response) {
      errorMessage.value = 'Такого пользователя не существует';
      loading.value = false;
      return;
    }
    ...
    errorMessage.value = 'Неверное имя или пароль';
  } finally {
    loading.value = false;
  }
};
...
/* Абсолютное центрирование карточки ККИ */
.glass-auth-card {
  position: absolute;
  top: 45%;
  left: 50%;
  ...
  /* УБРАЛИ СТРОКУ С BORDER */
  border: none !important; 
  
  border-radius: 24px;
  padding: 28px 20px;
  
  /* Сделали тень чуть мягче и объемнее для эффекта парения */
...
.custom-input {
  /* Полностью обнуляем ширину и цвет встроенной рамки Ionic */
  --border-width: 0px !important;
  --border-color: transparent !important;
  --border-style: none !important;
  
  /* Делаем мягкую серую подложку вместо рамки */
